...
Voting is the heart and basis of the American democracy. Every single vote is counted equally and it represents the will and voice from the people. If one thinks a single vote doesn't matter because it is just one vote in a billion, he/she could be wrong. In a tight election race, such as the 2020 presidential election, the elected candidates secured only slight more votes in the swinging states than their opponents by sometime less than 1%. This tiny lead eventually determined who was going to run the Office. Therefore, every single vote counts, and every single vote is important. Under such circumstances, the enthusiasm of the voters towards the presidential candidates is critical to higher turnout numbers, which affects the results of the election. As a matter of fact, understanding which candidate is more attractive/likable to the voters is crucial for the parties to adjust their election strategies/decisions.      
...
\
biden_rating, harris_rating, voted_early_in_gen_election, plan_to_vote_in_gen_election, and party_of_registration_str_edited
\
\
The two variables, biden_rating and harris_rating, were the main variables we used to infer the voter's enthusiastic rating in this study. The two variables were metric and (should) ranged from 0 to 100. We applied filters excluding samples that were outside of this range or with missing data. We also filtered the data down to Democratic voters only, using the following variables: voted_early_in_gen_election, plan_to_vote_in_gen_election, and party_of_registration_str_edited. In this study, we defined the Democratic voters as voters who (1) were registered as Democrats, (2) intended to vote for the Democrats, or (3) early-voted for the Democrats. The total voter counts were 2691 with all the filters applied. Fig. 1 illustrates the normalized histogram of the Democratic voters' enthusiastic ratings of Biden vs. Harris. The distribution of the ratings were very similar to each other. The ratings both peak between 80 and 90, with skewed distributions to the right, due to the constraint of the rating (capped at 100).
...
hist(h2$harris_rating, col=rgb(0,0,1,0.5),
     main = "Biden vs. Harris Voter Enthusiastic Rating", 
